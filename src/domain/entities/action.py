...
class Money(BaseModel):
    ...
    frequency: Optional[str] = Field(
        default=None, description="Payment frequency (e.g. ANNUAL, HOURLY)"
    )


class ActionRequest(BaseModel):
    parameters: Dict[str, Any] = Field(
        description="Key-value pairs of parameters for the action"
    )
    dry_run: bool = Field(
        default=False,
        description="If true, only validate and simulate the action without making changes",
    )
...
class Provenance(BaseModel):
    ...
    trace_id: str = Field(
        description="Unique identifier for tracing this request across systems"
    )
...
class SecurityContext(BaseModel):
    authorization_policy: str = Field(description="Which policy granted access")
    audit_level: str = Field(description="BASIC or VERBOSE logging")
    mfa_verified: bool = Field(description="Whether MFA was present")
    fields_filtered: bool = Field(default=False, description="Whether PII was filtered")
...
class ActionResponse(BaseModel):
    data: Union[Dict[str, Any], List[Any]] = Field(
        description="The actual result payload of the action"
    )
