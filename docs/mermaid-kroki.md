...
## Troubleshooting: C4 Diagrams Not Rendering

If your **Flowchart** and **Sequence** diagrams render correctly, but **C4 diagrams** fail, it is usually due to one of two issues specific to the embedded icons in C4 diagrams.

### Issue 1: Content Security Policy (CSP)

**Symptoms**: The diagram appears as a broken image icon or fails to load the embedded person/system icons.
**Cause**: C4 diagrams contain **embedded icons** (encoded as `data:image/png;base64...`). Backstage's default CSP blocks `data:` URIs.

**The Fix**:
Update `app-config.yaml` to allow `data:` in the `img-src` directive.
...
### Issue 2: Invalid XML (Namespace Error)

**Symptoms**: Opening the SVG in a new tab shows: `Namespace prefix xlink for href on image is not defined`.
**Cause**: The SVG generated by Kroki uses `xlink:href` for the embedded icons but fails to declare the `xmlns:xlink` namespace in the root `<svg>` tag. This makes the file invalid XML.

**The Fix**:
You must patch the generated SVG files after the build. Run this command after `mkdocs build`:

```bash
# Fixes all SVGs in the site directory
find site -name "*.svg" -exec sed -i 's/<svg /<svg xmlns:xlink="http:\/\/www.w3.org\/1999\/xlink" /' {} +
```

*Note: We recommend adding this to your CI/CD pipeline or build script.*
